...
└── tools/
    ├── xcodebuild/       # 8 tools — xcodebuild operations
    ├── simulator/        # 10 tools — xcrun simctl operations
    ├── app/              # 8 tools — app lifecycle on simulator
    ├── debug/            # 6 tools — logging, screenshots, recording
    ├── environment/      # 5 tools — location, push, status bar
